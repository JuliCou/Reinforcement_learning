...
input_num_units = taille_grille ** 2
hidden_layer_num_units = taille_grille ** 2
...
model = Sequential(Linear(input_num_units, hidden_layer_num_units),
                   ReLU(),
                   Linear(hidden_layer_num_units, output_num_units)
                   )
...
while epoch < epochs:
    ...
    while nb_couts < 100 and not(fin):
        ...
        etat = np.zeros(16)
        etat[int(taille_grille*pos[0] + pos[1])] = 1
        X = torch.tensor(etat, dtype=torch.float32).unsqueeze(dim=0)
        pred = model(X)
        ...
        etat_futur = np.zeros(16)
        etat_futur[int(taille_grille*pos_future[0] + pos_future[1])] = 1
        X_futur = torch.tensor(etat_futur, dtype=torch.float32).unsqueeze(dim=0)
        # Application du modèle à l'état q_etat_futur
        # pred_action_future = torch.argmax(model(X_futur)).item()
        # pred_future = model_stable(X_futur)[0][pred_action_future]
