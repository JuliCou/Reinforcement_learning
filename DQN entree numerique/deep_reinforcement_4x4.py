...
class QNetwork(nn.Module):
    def __init__(self, state_size, action_size, seed, fc1_units=30, fc2_units=10):
...
model = DQNModel(1, 4, 0.9, 1e-2, 1e-3)
epochs = 1000
...
for epoch in range(epochs):
    ...
    while nb_couts < 100 and not(fin):
        ...
        a_t = model.act(X, eps=epochs/(epochs+epoch))
...
while not(fin) and nb_couts < 100:
    ...
    a_t = torch.argmax(model(input_nn)).item()
